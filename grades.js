...
let grades = Array.from({ length: 12 }, () =>
...
let summ = count(grades);
let lnght = grades.length;
average = Math.floor (summ/lnght);
...
let bestGrades = grades.filter(function (grade) {
  return grade > 60;
});
console.log(
...
let lowGrades = grades.filter(function (grade) {
  return grade <= 60;
});
console.log(
...
let finalGrades = grades.map(function (number) {
